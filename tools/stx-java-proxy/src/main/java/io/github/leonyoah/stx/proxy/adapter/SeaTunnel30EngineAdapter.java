...
/**
 * Adapter implementation for SeaTunnel 3.0.x engine runtime. Provides resilient inspection for 3.0
 * lightweight/incremental checkpoint states, table-level source splits, ChangeStream SPI detection,
 * and savepoint flags.
 */
public class SeaTunnel30EngineAdapter extends AbstractSeaTunnelEngineAdapter {
    ...
    public String getAdapterVersion() {
        return EngineAdapterVersionPolicy.ADAPTER_V30;
    }
    ...
    public boolean supports(EngineVersion version) {
        ...
        return version.matches("3.0.*") || version.isVersion3x();
    }
        ...
        return runWithClassLoader(
                ...
                () -> {
                    ...
                    if (actionStates.isEmpty()) {
                        // Check 3.0 alternative incremental task state getter
                        actionStates =
                                extractMapQuietly(completedCheckpoint, "getIncrementalTaskStates");
                    }

                    Map<String, Object> taskStatistics =
                            extractMapQuietly(completedCheckpoint, "getTaskStatistics");

                    // 3.0 Incremental & Savepoint detection
                    boolean incremental = false;
                    Object incFlag = readPropertyQuietly(completedCheckpoint, "incremental");
                    if (incFlag instanceof Boolean) {
                        incremental = (Boolean) incFlag;
                    } else {
                        Object incFiles =
                                readPropertyQuietly(completedCheckpoint, "incrementalStateFiles");
                        if (incFiles instanceof Map && !((Map<?, ?>) incFiles).isEmpty()) {
                            incremental = true;
                        }
                    }
                    ...
                    // 3.0 Table-level states inspection
                    Map<String, Object> tableStates =
                            extractMapQuietly(completedCheckpoint, "getTableStates");

                    Map<String, Object> extra = new LinkedHashMap<>();
                    extra.put("engineVersion", "3.0.x");
                    extra.put(
                            "checkpointType",
                            checkpointType != null ? checkpointType.toString() : "CHECKPOINT_TYPE");
                    extra.put("tableLevelStateSupported", !tableStates.isEmpty());
                    if (!tableStates.isEmpty()) {
                        extra.put("tableCount", tableStates.size());
                    }
                ...
                () -> {
                    ...
                    for (Map.Entry<String, Object> entry : actionStates.entrySet()) {
                        ...
                        if (actionName != null && actionName.toLowerCase().startsWith("source")) {
                            ...
                            sourceInfo.put("isIncremental", checkpointData.isIncremental());
    ...
    public Map<String, Object> getEngineCapabilities() {
        ...
        caps.put("incrementalCheckpoint", true);
        caps.put("tableLevelState", true);
        caps.put("changeStreamSpiSupported", true);
