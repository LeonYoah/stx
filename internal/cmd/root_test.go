...
func TestRootCommandRegistersOnlyPublicServerEntry(t *testing.T) {
	...
	for _, name := range []string{"auth", "admin", "dashboard", "host", "cluster", "config", "package", "plugin", "monitor", "monitoring", "diagnostics", "skill"} {
...
func TestRootCommandRegistersDiagnosticsReadCommands(t *testing.T) {
	command := newRootCommand(func() error { return nil })
	paths := [][]string{
		...
		{"diagnostics", "task", "bundle"},
		{"diagnostics", "task", "html"},
		{"diagnostics", "task", "file"},
		{"diagnostics", "error", "group", "list"},
		{"diagnostics", "error", "event", "list"},
		{"diagnostics", "error", "group", "get"},
	...
	for _, path := range paths {
		...
		if err != nil || len(remaining) != 0 || found.Name() != path[len(path)-1] {
			t.Fatalf("诊断查询命令未完整注册: path=%v found=%s remaining=%v err=%v", path, found.CommandPath(), remaining, err)
		}
	}
}

func TestRootCommandRegistersMonitorCommands(t *testing.T) {
	command := newRootCommand(func() error { return nil })
	paths := [][]string{
		{"monitor", "config", "get"},
	...
	for _, path := range paths {
		found, remaining, err := command.Find(path)
		if err != nil || len(remaining) != 0 || found.Name() != path[len(path)-1] {
			t.Fatalf("进程监控命令未完整注册: path=%v found=%s remaining=%v err=%v", path, found.CommandPath(), remaining, err)
		}
	}
}
