...
func (m *mockHostUpdater) UpdateAgentStatus(ctx context.Context, hostID uint, ipAddress string, agentID string, version string, sysInfo *SystemInfo, hostname string) (uint, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
