...
func syncOperationSpecs() []OperationSpec {
	...
	specs = append(specs,
		...
		syncOp("sync.curated.update", nil, "Update a curated sync template", "PUT", "/api/v1/sync/curated-templates/:id", RiskR1, "更新精选模板内容。", syncWriteInputs(false, []InputSpec{{Name: "id", Location: InputPath, Required: true, Description: "Curated template id"}}), "stx sync curated update <id> --confirm", false, false),
		syncOp("sync.curated.delete", nil, "Delete a curated sync template", "DELETE", "/api/v1/sync/curated-templates/:id", RiskR1, "删除用户精选模板。", syncWriteInputs(false, []InputSpec{{Name: "id", Location: InputPath, Required: true, Description: "Curated template id"}}), "stx sync curated delete <id> --confirm", false, false),
